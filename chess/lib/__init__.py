...
# This is a gui function that draw
# s green squares marking the legal moves of
# a seleced piece.
...
def animate(win, side, board, fro, to, load, player=None):
    ...
    col = (113,111,169) if (fro[0] + fro[1]) % 2 else (232,165,201)
...
def showScreen(win, side, board, flags, pos, load, player=None, online=False):
    ...
    if load["allow_undo"]:
        win.blit(CHESS.UNDO, (2, 2))

    win.blit(CHESS.SPEAK[0], (145, 458))
    win.blit(CHESS.SPEAK[1], (190, 472))

    for i in range(8):
        win.blit(CHESS.X_AXIS[i], (73 + i * 50, 30))
        win.blit(CHESS.Y_AXIS[i], (30, 68 + i * 50))


    if isEnd(side, board, flags):
        if isChecked(side, board):
            win.blit(CHESS.CHECKMATE, (100, 6))
            win.blit(CHESS.LOST, (320, 6))
            win.blit(CHESS.PIECES[side]["k"], (270, 0))
        else:
            win.blit(CHESS.STALEMATE, (160, 6))
    else:

        if isChecked(side, board):
            win.blit(CHESS.CHECK, (200, 6))
